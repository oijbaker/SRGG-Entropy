...
int main(int argc, char *argv[]) {
    ...
    if (argc != 7) {
        cout << "Usage: " << argv[0] << "<eta> <L> <r0> <step_r> <step_theta> <interior_radius>" << endl;  
        return 1; 
    }
    ...
    string filename = "../data/mass_values_annulus_" + to_string(radius) + "_" + to_string(r0) + "_" + to_string((int) eta) + ".csv";
    cout << "Writing to file: " << filename << endl;

    // Open the file for writing
    ofstream file(filename);
    file << "r,theta,mass" << endl; // Write header
    for (double r = radius; r <= 1; r += stepr) {
        cout << "r: " << r << ", Progress: " << (r-radius)/(1.0-radius) * 100.0 << "%" << endl;
        for (double theta = 0; theta <= 2*3.14159; theta += steptheta) {
            double mass = local_entropy(r * cos(theta), r * sin(theta), r0, eta, L, radius);
