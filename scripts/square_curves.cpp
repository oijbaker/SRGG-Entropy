...
int main(int argc, char *argv[]){
    ...
    if (argc != 7) {
        cout << "Usage: " << argv[0] << "<N> <eta> <L> <r0min> <r0max> <step>" << endl;
        return 1;
    }
    ...
    string filename = "../data/entropy_values_square_" + to_string(N) + "_" + to_string((int) eta) + ".csv";
    ...
    for (double r0 = r0min; r0 <= r0max; r0 += step) {
        ...
        // Calculate entropy
        double entropy = entropy_graph(N, r0, eta, L);
